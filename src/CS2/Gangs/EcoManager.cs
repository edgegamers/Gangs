...
public class EcoManager(IServiceProvider provider) : IEcoManager {
  ...
  public async Task<int> TryPurchase(PlayerWrapper player, int balanceDue,
    ...
    try {
      ...
      switch (usedBank) {
        case true when usedPlayer:
          player.PrintToChat(localizer.Get(MSG.ECO_PURCHASED_WITHBOTH_ITEM,
            item, balanceRemaining));
          break;
        case true:
          player.PrintToChat(localizer.Get(MSG.ECO_PURCHASED_WITHGANG_ITEM,
            item, gangBalance));
          return gangBalance;
        default: {
          if (usedPlayer)
  ...
  private async Task<(int, int)> getBalance(ulong steam) {
    ...
    try {
      await semaphore.WaitAsync();
      var (success, balance) =
        await playerStats.GetForPlayer<int>(steam, statId);
