...
public class TargeterTests(IStringLocalizer locale) : TestParent {
  ...
  public async Task Multi_NoPlayers(IServerProvider _,
  ...
  [Theory]
  [ClassData(typeof(TargeterTestData))]
  public async Task Multi_Self_Me(IServerProvider server,
    IPlayerTargeter playerTargeter) {
  ...
  [Theory]
  [ClassData(typeof(TargeterTestData))]
  public async Task Multi_Self_Name(IServerProvider server,
    IPlayerTargeter playerTargeter) {
    await server.AddPlayer(TestPlayer);
    var players = (await playerTargeter.GetTarget(TestPlayer.Name!, TestPlayer))
     .ToList();
    Assert.Equal([TestPlayer], players);
  }
  ...
  [Theory]
  [ClassData(typeof(TargeterTestData))]
  public async Task Multi_Other_Name(IServerProvider server,
    IPlayerTargeter playerTargeter) {
    await server.AddPlayer(TestPlayer);
    var other = new PlayerWrapper(new Random().NextULong(), "Other Player");
    await server.AddPlayer(other);
    var players =
      (await playerTargeter.GetTarget("Other", TestPlayer)).ToList();
    Assert.Equal([other], players);
  }

  [Theory]
  [ClassData(typeof(TargeterTestData))]
  public async Task Single_NoPlayers(IServerProvider _,
    IPlayerTargeter playerTargeter) {
  ...
  [Theory]
  [ClassData(typeof(TargeterTestData))]
  public async Task Single_Me_All(IServerProvider server,
    IPlayerTargeter playerTargeter) {
    await server.AddPlayer(TestPlayer);
  ...
  [Theory]
  [ClassData(typeof(TargeterTestData))]
  public async Task Single_Me_Me(IServerProvider server,
    IPlayerTargeter playerTargeter) {
    await server.AddPlayer(TestPlayer);
    Assert.Equal(TestPlayer,
